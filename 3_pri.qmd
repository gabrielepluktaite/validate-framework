...
**In Progress**
<br>**Note**: tool complies with both national/local regulations and GDPR for data projection. The internal ethics review processes for collection of data for the prospective shadowing study involves appropriate review by local committees. The VALIDATE tool was not compliant with GDPR if integrated with hospital IT systems, and therefore the solution involved ``siloing'' of the VALIDATE tool. Specifically, the tool was kept separate from hospital systems, being used by clinicians via an app on a separate phone, which was connected to the server to transfer data on a WiFi network outside of the hospital. The solution enabled compliance with GDPR but was not scalable.
:::
...
**Fulfilled**
<br>**Note:**
This requirement addresses interoperability. A standard definition was applied, although no unique interoperability framework was specifically followed. Instead, the project used the OHDSI Common Data Model
as a reference and adapted it to meet project-specific needs. <a href="https://ohdsi.github.io/CommonDataModel/">See here</a>, also documented in D4.3 which is added as an artefact.
<br>**See Artefacts:**
...
::: {style="background-color:#e8daef; padding:6px 10px; border-radius:5px; display:inline-block;"}
**In Progress**
<br>**Note**: data management plan is scheduled for delivery in May 2026, and will be added to this requirement as an in-progress artefact.
:::
...
::: {style="background-color:#abebc6; padding:6px 10px; border-radius:5px; display:inline-block;"}
**Not Completed**
<br>**Note**: No data on ethnicity and race are recorded; some data are available in electronic health records, but collected unevenly and shouldn't be used (or shared). There is no differentiation between sex and gender in the collected data. Disability-related data will be included in the shared dataset, and no special risks of sharing this data in an anonymized format are foreseen. 
:::
